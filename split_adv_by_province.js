...
console.log(`\n=== Splitting ${year} advance poll data by province ===\n`);

const boundariesPath = `election_boundaries_19-25/${year}_boundaries/geojson/${year}_adv_wgs84.json`;
const outputDir = `election_boundaries_19-25/${year}_boundaries/geojson/adv_by_province`;
...
console.log(`Loaded ${boundaries.features.length} advance poll features`);
...
console.log(`\nGrouped advance polls into ${provinceBoundaries.size} provinces:`);
...
provinceBoundaries.forEach((features, pruid) => {
    ...
    const outputPath = path.join(outputDir, `${pruid}_${province.abbr}_${year}_adv.json`);
    fs.writeFileSync(outputPath, JSON.stringify(provinceGeoJSON));

    const fileSizeMB = (fs.statSync(outputPath).size / (1024 * 1024)).toFixed(2);
    console.log(`  ✓ ${province.name} (${province.abbr}): ${features.length} advance polls → ${fileSizeMB} MB`);

    filesCreated++;
});

console.log(`\n✓ Completed! Generated ${filesCreated} province-level advance poll files in ${outputDir}\n`);
